...
- **Раннер проверяется до work-этапа.** Нода `probe` резолвит головные слова
  validate-cmd (`command -v` в staged worktree, тем же login-шеллом, что и сам
  validate-cmd) сразу после setup. Не нашёлся бинарь — прогон падает за секунды
  с именем бинаря и подсказкой про `--setup-cmd`, вместо красного work-гейта
  через сорок минут и оплаченный agent-leg. Команда, которая ставит зависимости
  сама (`bun install && …`, `make …`), не проверяется: раннер появится по ходу,
  и ложный отказ дороже пропуска.
...
- **Нетерминальный `status` в отчёте ноги** — четвёртый класс, БЕЗ кода
  ...
  (`lib/review-merge.ts`) считает ногу с нетерминальным статусом failed —
  дальше работает штатная degraded/pause-семантика гейта вместо тихой
  ноги-с-нулём-находок. Терминальность статуса — `isTerminalReviewStatus`
  (`lib/review-schema.ts`), по границам слов: `SMOKE OK`,
  `reviewers complete`, `completed: <список>` — здоровые.
