...
- **Worktree прогона не содержит built dists workspace-пакетов.** Тест,
  ...
  чистом worktree, хотя в рабочем checkout зелёный. Либо не включай такие
  тесты в гейт, либо учитывай пре-билд (`bunx turbo run build
  --filter=<pkg>` в worktree чинит попытку, но следующая попытка стартует
  так же голой).
- **Spawn-чувствительные тесты флачат под движком.** Тест, который спавнит
  подпроцессы и зависит от окружения (ratchet-чекеры, форматтеры), может
  падать только под validate движка (подозрение — его sandbox) и проходить
  из любого шелла, включая `bash -lc`. Такой тест в гейте недиагностируем —
  исключай из validate-cmd.
