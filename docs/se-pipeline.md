...
- `--validate-cmd` — override; без него команду даёт сам план (`validate_commands`
  в frontmatter, иначе секция `Verification Contract`). Из конфига целевого репо
  пайплайн команды не читает никогда (KTD8). Гейт work-стадии исполняет её в
  worktree сам.
- **Work-агент получает замороженную копию плана, а не путь оператора.**
  Стейджинг сверяет план на диске с hash'ем gate-0 и кладёт копию РЯДОМ с
  worktree (`<worktree>-plan/`, `staging.planCopyPath`), промпт называет её.
  Внутрь worktree копию класть нельзя: `git add -A` work-гейта закоммитил бы её
  на ветку прогона и сделал бы tree-hash отличным от базового для плеча, которое
  ничего не сделало, — то есть отключил бы KTD14. Путь запуска
  (`gate0.planPath`) остаётся авторитетом только для повторного хеширования
  KTD7: эта проверка ловит правку настоящего плана посреди прогона. Копия
  удаляется вместе с worktree. Причина в `docs/solutions/design-patterns/absolute-paths-beat-prose-in-agent-isolation.md`.
- **Work-гейт называет побег из worktree, но не роняет по нему.** Стейджинг
  пишет digest от `git status --porcelain` главного чекаута; если он сдвинулся,
  гейт дописывает причину («агент мог писать ВНЕ worktree») и оставляет вердикт
  как есть — оператор, правящий свой чекаут в течение многочасового прогона, это
  норма, а красный гейт на ложном срабатывании стоит лишнего work-плеча.
...
- **Agent-блоку с `planPath` промпт называет замороженную копию плана, не путь
  оператора.** Стейджинг снимает hash плана (`planContentHash` — тот же, что у
  gate-0 пайплайна; у флоу своего gate-0 нет, поэтому момент стейджинга и есть
  точка заморозки), кладёт копию РЯДОМ с worktree (`<worktree>-plan/`, никогда
  внутрь: `git add -A` закоммитил бы её и сломал KTD14) и пишет
  `{blockId, planPath, planHash, copyPath}` в durable-строку `staging`. Каждый
  рендер перепроверяет копию по этому hash: план, отредактированный посреди
  прогона, — красная строка блока, а не диспатч агента по спеке, которую прогон
  не замораживал. Подстановка живёт в интерпретаторе (`renderBlock`), `buildPrompt`
  остаётся чистой функцией от входа блока.
- **Промпт каждого agent-блока (`work`, `repro`, `analysis`, `subtasks`)
  проговаривает конверт return-to-caller по полям.** `jsonSchema` агента
  намеренно описывает только обёртку `{report: string}` (сырая форма превратила
  бы кривой ответ в `INVALID_OUTPUT`-падение таски вместо читаемой красной
  строки), поэтому содержимое строки надо ПРОСИТЬ. Список полей берётся из
  `workEnvelopeSchema` в рантайме — промпт и парсер не могут разъехаться.
...
`waive` решает, что происходит с наследниками блока, если его гейт стал
красным после того, как ретраи израсходованы. `none` — блок останавливает
всё, что от него зависит, по `after` и `bindTo`, транзитивно; независимые
ветви DAG продолжают идти. `approval` — прогон паркуется на Approval у этого
блока, и наследники ждут решения оператора; отказ останавливает прогон.
Нерешённая пауза удерживает: неизвестное решение никогда не выпускает красный
блок. Вердикт прогона в outcome-record называет блок, который его остановил
(`stopped-by-red-gate` / `parked-for-waive-approval`), и эпилог отрабатывает в
любом случае — иначе остановленный прогон остался бы без outcome-record.
`compute`-блокам каталог разрешает только `none`, поэтому красный
`secret-scan` нельзя waive'ить никаким спеком.
...
**По умолчанию команда берётся из самого плана** (gate-0). Ничего передавать не
нужно:
```bash
se pipeline docs/plans/<план>.md          # validate берётся из плана
```

Форм две, и они проверяются в этом порядке.

**1. Декларация — основная форма.** План выписывает свои команды YAML-списком в
frontmatter, рядом с `artifact_readiness`:
...
`/se-plan` кладёт этот список в план сам — узкие, с таймаутами команды из
Verification Contract того же плана. Список исполняется **дословно**, каждая запись в своём сабшелле
(`(a) && (b)`). Keep-фильтр к декларации не применяется: «похоже ли это на
верификацию» — не догадка пайплайна, когда оператор написал команду сам (KTD8,
план — доверенный вход). Незнакомый раннер просто запускается, и в notes прогона
про него ничего нет. Остаётся одна проверка — **мутирующий флаг**
(`--write`, `--fix`, `-i`, `--apply`, …): work-гейт после validate-cmd требует
чистое дерево, поэтому форматтер в гейте роняет стадию по причине, не связанной
с работой. Такая запись отклоняет **весь** запуск на gate-0 с названным флагом —
каждая строка декларации намеренная, и выполнить остаток значило бы проверить
меньше, чем объявлено, и уйти в зелёный. Догадки по ИМЕНИ (`prettier`,
`bun run fmt:check`) декларацию не блокируют — блокирует только флаг.

Сломанная декларация (скаляр вместо списка, flow-список `[a, b]`, пустой
список, пустая запись) — тоже отказ на gate-0 с указанием, как её починить.
Отката к секции при этом нет: иначе прогон исполнил бы не то, что объявлено, и
молча.

**2. Секция `Verification Contract` — fallback-парсер.** Ключа в frontmatter нет
→ команды вычитываются из секции (канонично `## `-заголовок; вложенный
`###`–`######` тоже принимается — секция читается до заголовка того же или более
высокого уровня, поэтому вложенный контракт не поглощает соседние секции).
Так написаны все планы, уже лежащие в целевых репозиториях, — миграции нет.
...
из фразы «script is `e2e`, not `test`». Извлекаются только исполнимые
check/test/typecheck строки; **отбрасываются** `storybook`/`--watch`/e2e/playwright
(сервер/браузер) и `fix`/`format` (мутируют worktree → уронили бы
clean-tree-проверку). Ручные/VRT строки игнорируются.

Пайплайн печатает выбранную команду и её источник в лог: `work-gate validate-cmd
[plan frontmatter validate_commands]: ...` или `[plan Verification Contract]: ...`.
Если ни одна форма не дала команды — gate-0 роняет прогон сообщением, которое
называет обе формы и то, что именно было отброшено.

`--validate-cmd '<cmd>'` — **только override** (или для планов без обеих форм).
KTD8: из плана (доверенный вход) брать безопасно; из конфига целевого репо —
по-прежнему нельзя (чужой коммит исполнил бы произвольное).

Если пишешь `validate_commands`, Verification Contract или override — команда
должна быть
