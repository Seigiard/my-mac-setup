...
The trap is that this suite — like most — treats a missing tool as a **skip, not a failure**. The bats idiom is `command -v <tool> || skip "..."`, and bats exits 0 on a skipped test. The plan's key decision KTD6 (`docs/plans/2026-08-20-2217-perf-ci-minimal-brew-install-plan.md:84`) counts 127 skip sites in `tests/*.bats`, 88 gated on `jq` alone. So "the suite passed on the minimal render" cannot distinguish a correct minimal set from one that silently stopped exercising dozens of tests.
...
- Recorded outcome (U2 "Verification outcome (2026-08-21)", `:183-192`): both renders produced **367 ok / 1 not ok / 15 skipped**, skip sets identical line for line; the single failure was the same pre-existing harness gap in both renders (already filed as `docs/issues/2026-08-19-001-make-test-ubuntu-fails-two-tests-on-main.md`), so it did not mask anything.
...
- **Count skipped tests, never skip sites** (session history): a guard inside `setup()` runs before every test in the file — `tests/palette.bats:14`'s `command_exists python3 || skip` silences all 56 tests in that file from one site. A site count understates the coverage at risk by an order of magnitude; the discovery is filed as `docs/issues/2026-08-21-009-python3-is-both-required-and-optional-in-the-test-suite.md`, which credits the parity gate — not ordinary CI — with surfacing it.
...
- Later, PR #27 (`f83e95d`) moved `docker/Dockerfile.ubuntu` to `FROM ubuntu:24.04` (`docker/Dockerfile.ubuntu:6`), whose git 2.43.0 accepts zdiff3 — the header comment at `docker/Dockerfile.ubuntu:1-5` documents exactly this chain.
...
And in any suite with **conditional skips**: bats `skip`, pytest `skipif`/`importorskip`, jest `test.skip`/`describe.skip`, Go `t.Skip`. The proof obligation is always "**the same tests ran**", not "the suite is green":
...
- Fix 1 (PR #26, `1e4ea73`): fold `git` back into the minimal Brewfile — `home/private_dot_config/brewfiles/Brewfile.tmpl:35-36`.
- Fix 2 (PR #27, `f83e95d`): `docker/Dockerfile.ubuntu:6` → `FROM ubuntu:24.04` (git 2.43.0), rationale comment at `:1-5`.

**The regression guard:** `assert_minimal_brewfile` in `tests/templates.bats:352-367` asserts the evidence-backed entries (`git`, `jq`, `grc`, `node`, `oven-sh/bun/bun` + tap) and refutes guarded ones, with the per-entry evidence in the comment at `:343-351`.
