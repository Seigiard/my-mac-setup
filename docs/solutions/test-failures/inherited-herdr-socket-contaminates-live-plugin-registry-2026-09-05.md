...
- The same class reappeared through `chezmoi apply` itself: an apply under a throwaway `$HOME` ran `run_onchange_after_5-link-herdr-caffeinate.sh.tmpl`, which registered that temp path in the live registry and left `keepawake.caffeinate` reporting `manifest unavailable` after the directory was collected. `home/.chezmoitemplates/herdr-plugin-link-guard.sh` now answers the question `$HOME` cannot answer about itself — it reads the login home from the account database and skips the link from any other home, fail-closed when the database cannot be read. The three link scripts (command-palette, worktree-setup, caffeinate) include it; `tests/bashunit/scripts_test.sh` tests 853 and 854 pair the refusal with a control that reaches the link.
