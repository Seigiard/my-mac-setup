...
- Not applicable when the orchestrator genuinely is a long-lived process (a daemon, a durable workflow engine, a CI runner) — those can hold subscriptions and poll. That is exactly the sibling case: external review legs are non-interactive one-shot subprocesses supervised by an engine that outlives them (idle timeouts on output bytes, wall-clock caps, fail-closed report parsing — `docs/solutions/design-patterns/external-review-legs-as-unreliable-subprocesses.md`). That toolkit presumes a supervisor that survives; an LLM parent's turn does not, so the direction inverts.
...
- Consciously accepted risks, filed as open issues rather than left implicit:
  - [#216](https://github.com/Seigiard/my-mac-setup/issues/216) — the `"*": "allow"` external-directory grant is machine-wide, covering the user's own sessions; the sandbox that will contain it is the named next piece of work (decision at plan:51).
  - [#215](https://github.com/Seigiard/my-mac-setup/issues/215) — children inherit the shared permission mode (bypassPermissions); read-only withholds tools without enforcing a write boundary (decision at plan:55).
  - [#214](https://github.com/Seigiard/my-mac-setup/issues/214) — the per-child watcher now covers ordinary lifecycle wakes; this issue remains for crash/restart resilience and a global event subscriber.
  - [#232](https://github.com/Seigiard/my-mac-setup/issues/232) — no ownership enforcement: any parent-side session can `reply`/`reap` any child; the callback markers and env are coordination data, not credentials.
  - [Frozen `2026-08-18-003` source record](https://github.com/Seigiard/my-mac-setup/blob/27f33a235548f19422b94565f6a14613219b5d5b/docs/issues/2026-08-18-003-headless-peer-consult-outside-herdr.md) — a declined capability the contract deleted (headless peer consult) with no replacement.
