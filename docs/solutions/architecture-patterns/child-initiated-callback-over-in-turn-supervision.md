...
- **The child initiates the return.** `herdr-child ask` submits a marked message into the parent's pane via `herdr agent prompt` (`home/dot_local/bin/executable_herdr-child:284-316`), "which starts a turn in the parent's session — the only path from a child back to a parent that does not depend on the parent still being inside its own turn" (plan:52, R16). The key decision explicitly rejects "any mechanism that wakes the parent without the child acting: every such mechanism needs a process that outlives the parent's turn" (plan:52).
- **`ask` itself never waits.** It returns as soon as delivery succeeds (KTD9, plan:272): "waiting inside the current turn would be the same trap this work exists to escape." The script prints "Question delivered. End this turn and wait for the parent reply." (`executable_herdr-child:315`) — the child's turn ends too; the reply starts a new one.
- **Passive state covers observation.** Before sending, `ask` publishes a sidebar state label `blocked=waiting for parent` with a 1-hour TTL (`executable_herdr-child:7,293-294`; R18, plan:58) — a waiting child runs no loop, so it cannot refresh a short-lived label; the parent clears it in the same step as the reply (`executable_herdr-child:346-350`), and the TTL is the backstop for a parent that never returns.
- **The parent's verbs are all fire-and-return.** The coordination verbs are `start` / `ask` / `reply` / `reap` (dispatch table at `executable_herdr-child:454-459`, plus only help and an unknown-subcommand error): launch, be called back, answer, and clean up settled panes on a later turn (`reap_children`, `executable_herdr-child:354-449`, which fails closed — keeping panes that are focused, still working, carrying a waiting label, or whose state cannot be parsed). There is no `wait` verb anywhere in the contract.
- **Failure also ends in a stop, not a wait.** A failed `ask` exits non-zero and leaves the label published (`executable_herdr-child:297-313`); the child's duty is to "stop instead of guessing" (`home/private_dot_claude/shared/child-agent-contract.md:128`).
...
- **Question dialogs are removed per-kind at launch.** claude children get `--disallowed-tools AskUserQuestion` (`executable_herdr-child:185,188`); opencode children get `OPENCODE_PERMISSION` with `"question":"deny"` set as a pane environment variable (`executable_herdr-child:151-160`); pi children get `--exclude-tools ask_user` in the native args (`executable_herdr-child:201`; R13, plan:86). The child's initial prompt reinforces it: "Do not open an interactive question dialog" (`executable_herdr-child:242`).
...
- **Removing the stall does not remove the decision.** "Removing the child's question tool removes the stall, not the decision the child was stalling on" (plan:56) — so the launch prompt substitutes the escalation path: "If you need a question or blocking decision, run: herdr-child ask '<decision brief>'" (`executable_herdr-child:242`; child duties in `child-agent-contract.md:123-128`).
...
- **Callback mechanism**: `ask_parent()` at `home/dot_local/bin/executable_herdr-child:284-316` — publish waiting label, verify a live parent occupies the launch pane, inject `[child-ask v1 agent=<name> pane=<pane-id>]` + body, exit without waiting.
- **Reply-and-clear as one step**: `reply_child()` at `executable_herdr-child:318-352` — the label clear happens only after delivery succeeds; a failed delivery leaves the label standing as "the last visible sign that the child is still waiting" (plan:508).
- **Reap on demand, fail closed**: `reap_children()` at `executable_herdr-child:354-449` — closes only panes whose agent reads `done`/`idle`, unfocused, no waiting label, re-checked against a fresh `herdr agent list` immediately before close (hardened in d8368be).
- **Launch-time suppression matrix**: posture and native-option-mapping tables at plan:340-364; implementation at `executable_herdr-child:151-207`.
...
- `docs/issues/2026-08-18-020-replace-deleted-herdr-pair-pattern-reference.md` — done: the herdr-pair deletion's plan-reference fallout; herdr-pair is the named prior art whose whole state machine lived inside one driver turn — the shape this learning replaces.
