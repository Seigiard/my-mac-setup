...
- R1. The Docker image contains all formulae from the full render of `home/private_dot_config/brewfiles/Brewfile.tmpl` at build time.
...
- **Landing order — this plan is third of four, and it lands after the template rename.** Order settled 2026-08-21 in `docs/issues/2026-08-21-002-perf-plan-landing-order-undecided.md`: CI-workflow-hygiene `concurrency` block, then CI-minimal-brew-install, then this plan, then the CI-workflow-hygiene download cache.
  ...
  **This plan owns the render step.** The CI-minimal-brew-install plan renames `Brewfile` to `Brewfile.tmpl` and wraps entries in chezmoi conditionals (`{{ if not (get . "ci_minimal") }}`). `brew bundle` parses a Ruby DSL and raises on that syntax, so `COPY`ing the renamed source and running `brew bundle` on it **fails the image build outright**. Since that plan lands first, U2 cannot be implemented as currently written. Two additions are required and belong here, not upstream:
  - Render before the bake: `COPY` `Brewfile.tmpl` into the image and render it to a plain `/tmp/Brewfile` with `chezmoi execute-template --file`, in the layer above `brew bundle`. Full render, no `ci_minimal` set — KTD5 keeps full-Brewfile fidelity in the container.
  - **Corrected 2026-08-21 during implementation — the render needs no allowlist widening.** An earlier draft of this bullet required `.chezmoi.yaml.tmpl` and `.chezmoitemplates` in the build context "so the render has its inputs". Measured: with `HOME` pointed at an empty directory and no chezmoi config present at all, `chezmoi execute-template --file Brewfile.tmpl` exits 0 with empty stderr and emits all 37 formulae. It needs neither file. `Brewfile.tmpl` reads the CI-minimal switch through `get . "ci_minimal"` — a sprig dict lookup that yields an empty string for a missing key rather than tripping chezmoi's `missingkey=error` — and it never references `.name` or `.email`. So the KTD4 allowlist stays at exactly three lines. Adding entries no build instruction reads would be dead configuration, and the absent config is what *selects* the full render, which is the KTD5 outcome this plan wants. See KTD6 for the guard that keeps that implicit dependency honest.
...
- KTD6. **Assert the full render at build time rather than plumbing `ci_minimal` explicitly.** The in-image render selects the full Brewfile *by absence*: no chezmoi config exists during the build, so `get . "ci_minimal"` yields empty and `$full` is true. That is correct but implicit, and a silent flip to the minimal render would drop KTD5's fidelity guarantee without failing anything — the image would simply bake seven formulae, and every test would still pass. The build therefore greps the rendered file for a formula only the full render contains and fails the layer if it is absent. Chosen over reproducing chezmoi's config through `execute-template --init`, which would need `.chezmoi.yaml.tmpl` in the context, `CHEZMOI_NAME`/`CHEZMOI_EMAIL` at build time, and a guarantee that a stray `MMS_CI_MINIMAL` never leaks into the build — three new failure modes to make explicit what one grep already proves.
...
- **Approach:**
  1. After the existing tool installs, add `COPY --chown=testuser home/private_dot_config/brewfiles/Brewfile.tmpl /tmp/Brewfile.tmpl`, then one `RUN` that renders, asserts, bundles, and drops the download cache: `chezmoi execute-template --file /tmp/Brewfile.tmpl > /tmp/Brewfile`, the KTD6 full-render assertion, `brew bundle --file=/tmp/Brewfile`, and `rm -rf "$(brew --cache)"`. Rendering inside the build rather than staging a rendered copy from the Makefile is the same argument KTD2 makes about the Brewfile itself: a direct `docker compose -f docker/docker-compose.yml build` must not depend on a Make target having run first. The cache removal is in the same `RUN` on purpose: the image sets `HOMEBREW_NO_INSTALL_CLEANUP=1`, so brew never purges `~/.cache/Homebrew`, and whatever bottle tarballs are on disk when the layer ends are committed permanently — with `ffmpeg`, `imagemagick`, `poppler`, `resvg`, and `node` in the set that is a large duplicate payload. Removing the cache manually does not reintroduce the race `HOMEBREW_NO_INSTALL_CLEANUP` guards, which is about post-install cleanup, not the download cache. Keep `/tmp/Brewfile` in the image — U2's verification and the Verification Contract both read it.
...
- **Test scenarios:**
  ...
  - **Over-broad-`COPY` probe:** change the *contents* of `home/private_dot_config/brewfiles/empty_Brewfile.macos.tmpl` — the macOS Brewfile's real source name, in-context under the KTD4 allowlist, never `COPY`'d — then `make build-docker` is fully cached. Change the contents, do not `touch`: Docker keys `COPY` layers on file content, not mtime, so a `touch` passes this probe vacuously whether or not the `COPY` is over-broad.
...
- **Baseline recorded 2026-08-22 on commit `5f171c9`, before U1 landed** (R6). Warm image, `make build-docker` fully cached at 1 s, two consecutive `make test-ubuntu` runs:

  | Run | Total | `brew bundle` portion | Residual (Oh My Zsh + `mise` + fff-mcp) |
  |---|---|---|---|
  | 1 | 223 s | 119 s | 8 s |
  | 2 | 235 s | 133 s | 9 s |

  So `brew bundle` is **53–57 % of a repeat run**, and the residual the Scope Boundaries defer to a follow-up plan is only 8–9 s — an order of magnitude smaller, which confirms the deferral was the right call. The theoretical floor after the bake is about **100–105 s** per run. Both runs reported 36 `Installing` lines, so the container genuinely reinstalled the set each time.

  **An earlier baseline on `9f539b5` (203 s / 197 s) was discarded, not merely superseded.** `main` moved 21 commits between the two measurements, and `06ac9d9` changed the installed set itself — `grc` out, `rgrc` plus the `lazywalker/tap` tap in — while the Makefile added a `test-issues` prerequisite to `test-ubuntu`. A before/after comparison against those numbers would have measured the rebase as well as the bake.

  **Both baseline runs failed the same two bats tests**, identically in both runs: `not ok 32 focus-notify plugin compiles and declares no build step or extra backend` and `not ok 72 Pi brew auto updater focused tests pass`. Both are pre-existing on `main` and unrelated to this plan; they are already tracked in `docs/issues/2026-08-19-001-make-test-ubuntu-fails-two-tests-on-main.md`, and the second has its own root-cause issue in `docs/issues/2026-08-21-011-pi-brew-test-unresolvable-path-in-docker.md`. Consequence for this plan's gates: `make test-ubuntu` and `make test-docker` cannot reach green before or after the change, so the usable gate is **exactly these two failures and no others**, not an all-green run.
...
1. ~~**Landing order across the four same-date performance plans is undecided.**~~ **Settled 2026-08-21** — see Scope Boundaries and `docs/issues/2026-08-21-002-perf-plan-landing-order-undecided.md` (closed). The CI-minimal-brew-install plan lands first, so `Brewfile` is already `Brewfile.tmpl` when this plan starts and this plan owns the `chezmoi execute-template` render step. **Superseded 2026-08-21:** this item also claimed the render "needs chezmoi data (`name`, `email`) available at image-build time" and that the `.dockerignore` allowlist had to widen to supply it. Both were wrong — measured during implementation, the render needs no config at all. See the corrected bullet in Scope Boundaries and KTD6.
2. ~~**Should the container keep full-Brewfile fidelity (KTD5)?**~~ **Settled 2026-08-21, before implementation — KTD5 stands.** The container keeps the full render; `MMS_CI_MINIMAL` is not set on `test-quick`. The rejected alternative was setting `MMS_CI_MINIMAL=1` there, which reaches the same repeat-run speed while dropping U1, the `.dockerignore`, and the Dockerfile change entirely. Reason: the Docker path is this repo's only place where the full cross-platform Brewfile is exercised end-to-end on Linux, and the CI-minimal plan's own R5 depends on that staying true while push/PR CI goes minimal. KTD6 adds the build-time assertion that keeps this settled answer from silently eroding.
