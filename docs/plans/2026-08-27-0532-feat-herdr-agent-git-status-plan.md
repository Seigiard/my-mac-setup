...
- Does pi ever move a session into another directory mid-session? Source reading found `ctx.cwd` fixed at session start and no rebinding mechanism. If that holds, pi's worktree column is already correct today and needs no work at all. U1 confirms by observation.
- Does opencode's `session.updated` actually carry the new `location.directory` after a worktree move? The schema carries the field; that a move updates it is inferred from documentation, not read in source. U1 confirms.
...
- **Files:** `home/dot_local/bin/executable_herdr-task-sync`, `tests/scripts.bats`, `tests/helpers/herdr_task_sync.bash`; plus the reporter for each kind U1 confirmed — `home/private_dot_claude/hooks/executable_statusline.sh` (and `home/private_dot_claude/private_settings.json.tmpl` if an idle `refreshInterval` proves necessary), `home/private_dot_config/opencode/plugins/herdr-task-sync.ts`, `home/dot_pi/agent/extensions/herdr-task-sync.ts`.
- **Approach:**
  1. Read the recorded agent-reported directory first in `resolve_pane_location`, and keep the existing pane-cwd path underneath it as the fallback (KTD3). Add the reporting side only for the kinds U1 confirmed, each through its own channel (KTD6), and nothing beyond what U1's evidence named — a kind U1 found needs nothing gets no code.
