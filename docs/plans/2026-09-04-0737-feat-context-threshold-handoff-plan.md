...
- Unverified until U1: whether a `command`-type Stop hook fires in `claude -p` and what marks such a session; whether a Stop hook's `systemMessage` renders to the operator; how long goal extraction takes on a near-threshold fork; how many consecutive `continue: false` returns survive the block cap; whether the `SessionStart` `source: "compact"` payload carries the PreCompact `session_id`. Each one is load-bearing for a decision above.
- The turn-count baseline is not yet comparable to the counting rule. The measurement swept whole transcript files, including subagent files and across compaction boundaries; KTD4 counts post-compaction assistant entries in the main transcript only. U1 re-derives it (R19).
...
- KTD7. **Non-interactive detection keys on a positive signal, never on a missing state file.** U1's first probe identifies the payload or environment marker that distinguishes a `claude -p` session; that marker is the test. Absence of a statusline state file is reserved entirely for KTD3's fullness-unavailable path, because the two conditions are indistinguishable from the file alone and reading absence as "headless" would silence a resumed interactive session — the long-session case this feature exists for. If U1 finds no positive marker, the fallback is to treat a session id that has *never* published state as non-interactive while a stale-but-present file stays interactive, and that weaker rule is recorded as a known gap. If U1 shows Stop hooks do not fire under `claude -p` at all, this decision collapses to nothing and is recorded as such. Governs R21.
...
- KTD12. **The hard-threshold repeat interval is a function of distance past the threshold, not a counter.** Deriving the next announcement turn from the current turn count and the threshold keeps the announcement file free of a decaying counter that a resume or fork could carry stale, and makes the cadence reproducible from state the hook already reads. Repeated `continue: false` returns will eventually meet Claude Code's block cap; R20's unconditional `systemMessage` is what keeps the crossing visible past that point, and U1's cap probe says where it starts. Governs R25.
