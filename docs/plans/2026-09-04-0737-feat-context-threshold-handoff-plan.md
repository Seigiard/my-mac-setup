...
- R26. What the extractor is given is bounded by a configured budget rather than by the length of the session. The extraction model's context window is smaller than the sessions this feature fires for, so an input that grows with the session is rejected on exactly the sessions that need a goal. *(Added after deployment: R13 said which model extracts the goal and how long the operator waits, and nothing said how much input that model is handed. See the amended U1 assumption below.)*
...
- Measured in U1 — goal extraction on a near-threshold fork takes **8–9 seconds** with `--model haiku`. Three runs against real 349- and 350-turn sessions of 1.0 MB and 4.8 MB returned in 8.9 s, 8.2 s, and 8.3 s, each with a usable one-line goal. Command: `claude -p '<extraction prompt>' --resume <session-id> --fork-session --model haiku`. This sets KTD11's extraction-bound default at 30 s, with generous headroom over the measurement, and U4's declared hook `timeout` above that. ***Amended after deployment: this measured the extraction's latency and never its input size, and the sessions it sampled happened to fit. In live use both extractions failed. A 391-turn session carried 309,577 tokens against `haiku`'s 200,000-token window, and `--resume --fork-session` returned `Prompt is too long` in 3.2 s — the failure is structural, because the turn threshold selects long sessions and a long session is what does not fit. Extraction now reads a bounded excerpt of the transcript instead of resuming the session (R26); the same session then extracted a usable goal in 14.5 s. The latency figure and its 30 s bound still stand.***
...
- Observed in U4's live run — the Stop hook reads the transcript before the turn's own assistant entry is durable in it, so the count it sees is one behind the turn that is ending. A fresh session's first Stop therefore counts zero turns. This shifts every turn-count threshold by exactly one turn and changes nothing else; it is recorded rather than corrected, because reconstructing the in-flight turn would mean parsing state the payload does not carry.
- Confirmed in U4's live run — `"matcher": "*"` on a `Stop` entry does not prevent the hook from firing, and `CLAUDE_CODE_ENTRYPOINT` reads `cli` in a real interactive TUI session, as U1 measured. Both were checked directly after an early probe was misread as a matcher problem.
...
- **Chosen values and why:**
  - `CONTEXT_USAGE_TURNS_WARN = 150`. U1's re-derived distribution puts p90 at 179 turns, so 150 warns roughly one session in eight (12.4% run past it). Lower makes the announcement routine; higher and the common expensive session is most of the way through before anything is said.
  - `CONTEXT_USAGE_TURNS_HARD = 300`. 3.1% of sessions reach it under the rule the hook evaluates. It is the same round number the original cost measurement identified as the expensive cohort, and it stays comparable to that figure now that both are counted the same way.
  - `CONTEXT_USAGE_FULLNESS_WARN_PCT = 70`. The measured average request context is 129k tokens — about 13% of a 1M window, 33% with the allowance — so fullness stays silent through nearly every session by design. It exists for the short session that pulled in several very large files (AE2). At 70 there are still roughly 300k tokens of room to act in.
  - `CONTEXT_USAGE_FULLNESS_HARD_PCT = 85`. Around 150k tokens of headroom left, which is enough for a compaction to run. Above that the margin stops being a margin.
  - Cadence: first gap 8 turns, halving for every 25 turns past the turn limit and every 3 points past the fullness limit, floor 1. Eight turns is a few minutes of work — often enough to be noticed at the moment of crossing, rare enough not to be noise. It reaches every turn at 375 turns and at 94% full, where losing the session is imminent. Both curves come from distance past the threshold, so nothing decaying is stored (KTD12).
