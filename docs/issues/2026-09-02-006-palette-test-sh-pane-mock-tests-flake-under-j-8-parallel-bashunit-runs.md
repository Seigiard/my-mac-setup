...
Running 'tests/lib/bashunit -j 8 tests/bashunit/palette_test.sh' intermittently fails 3 tests around lines 840, 855, and 967 (the R4 pane-run tests at 840/855 and the R6 report-metadata test at 967) with output like 'Could not read pane w1:p1, so Run there was not run.' and 'cat: .../overlay/herdr.log: No such file or directory'. Rerunning the identical command with -j 8 immediately after often passes clean (57 passed, 1 pre-existing risky, 194 assertions). A single-threaded run ('-j 1') passed clean as well. Confirmed via 'git stash' that the same failure pattern reproduces on main (commit b632c34) with none of this branch's edits applied, so this is pre-existing test-harness flakiness, not something introduced by docs/issues/2026-09-02-003. Suspected cause: the pane-mock fixture these tests share (pane id w1:p1, an overlay/herdr.log file under a per-test PALETTE_WORK tmp dir) has state that is not fully isolated or not fully synchronized before assertion under -j 8 parallel worker contention.
