...
Scope narrowed by `docs/issues/2026-08-14-014-plan-format-contract.md` (2026-08-15): the filter now runs only on the prose-parsed path. A plan that declares `validate_commands:` in its frontmatter skips the keep side entirely — the operator wrote the list down, so "does this look like a verification" is no longer a question — and keeps only the flag-detected mutation refusal, which is physics (the work gate requires a clean worktree), not classification.
