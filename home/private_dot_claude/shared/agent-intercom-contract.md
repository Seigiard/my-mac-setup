...
- **its pane had no record and no name could be taken at all.** A plain-shell Claude launch in a fresh pane no longer lands in the fallback: the launcher declares a pane agent record, names it from the pool, and enrolls under that name. A relaunch in a pane that already hosted a Claude session cannot declare anything — Herdr keeps that pane's `claude` agent-session identity and ignores the declaration — so the launcher keeps the name it allocated and defers the record's rename to the first prompt. OpenCode and Pi still land here by design — the claim suppresses Herdr's screen detection until something releases it, and the only release surface this repository deploys is Claude Code's first-prompt hook, so a claim taken for them could never be given back. Claude lands here too when `herdr` or the allocator is missing, when the pool answers only with placeholders, when three successive names are taken, or when neither the claim nor the pending rename can be recorded on disk.
