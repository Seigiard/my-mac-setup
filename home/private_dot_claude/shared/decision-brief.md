...
The shape of every request for a user decision — a paused gate, an open finding in a review envelope, a plan ambiguity. The reader has not followed the run: they have not read the log, the envelope, or the plan, and they do not know the vocabulary the run used internally.
