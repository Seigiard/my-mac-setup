...
Every agent pane or tab you create follows one lifecycle, regardless of how it was launched: start → arm a wait → collect and verify the result → close. Close `herdr-child` children — including ones started with `--tab` — with `herdr-child reap`; close any other manually assembled pane or tab (`tab create` + `agent start` outside `herdr-child`) with `herdr pane close` or by closing its tab. The turn that reports a child's result to the user is not finished until its pane is closed or the exception below is invoked.
...
- Add `--tab [--label TEXT]` when the child needs its own tab instead of a split pane in yours. It requires `HERDR_WORKSPACE_ID` and is mutually exclusive with an explicit `--direction`. Start's JSON then carries a `"tab"` key alongside `agent`/`pane`. `--label` is presentation-only best-effort text — the deployed pane-label reconciliation sweep may rename the tab within one sweep interval.
- `herdr-child reap` closes a `--tab` child's tab only when the child pane is its only pane (herdr's own last-pane-close auto-close makes this a plain pane close, not a separate tab-close step); when the tab holds sibling panes, reap closes only the child pane and reports the tab kept with its pane count.
