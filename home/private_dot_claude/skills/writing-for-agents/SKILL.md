...
description: Writing documents and prompts for agents. Use when creating or editing skills, modifying AGENTS.md or CLAUDE.md, or writing a task brief or prompt that dispatches another agent (subagent, opencode session, pipeline leg).
---
...
A **dispatch brief** — the prompt handed to a spawned agent (a subagent, an opencode session, a pipeline leg) — is a document with exactly one run, read by an agent whose context is empty. The receiver has none of your session: no plan discussion, no accepted decisions, no earlier turns. The brief is its sole source of truth.

- Inline every decision as an instruction carrying its own rationale: "use X because Y". A reference to where the decision happened — "as agreed", "per the review", "the approach we chose" — is a context pointer whose target the receiver cannot reach; it resolves to nothing and the agent improvises.
