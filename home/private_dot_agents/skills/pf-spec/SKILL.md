...
Build the spec narrative HTML (pf-cycle → artifact storage + HTML mechanics + Language, which makes this page English; visible text follows pf-cycle → Naming on public surfaces — the page presents itself as the epic's contract spec) from what the PR actually changed — never from intentions:
